/******************************************************************************/
/*!
\file		Level1Controller2.h
\author 	Brandon Hsu
\par    	email: b.hsu\@digipen.edu , 620007421
\date		Jan 20, 2023
\brief		Controller Script for Level 1 area 2
...
namespace Level1Controller2 {
	std::string SKey{ "Level1Controller2" };
	...
	void Init(GameObject* GO, float dt) {
		...
		if (CT != nullptr) {
			if (MasterObjectList->StoredController != nullptr) {
				if (MasterObjectList->StoredController->LevelTag == "Level1_2") {
					...
					//Set player position to correct "side"
					MasterObjectList->ClearController();
					P->GetComponent<Transform>()->Position.x = Engine->GetGameObject("ArrowRight")->GetComponent<Transform>()->Position.x - 100.f;
					P->GetComponent<Transform>()->Dimensions.x = -P->GetComponent<Transform>()->Dimensions.x;
					Engine->GetObjectComponent<Transform>(Engine->GetObjectByPrefix("Camera"))->SetPosition(P->GetComponent<Transform>()->Position);
				}
				else if (MasterObjectList->StoredController->LevelTag == "Level1_4") {
					//Set player position to correct "side"
					MasterObjectList->ClearController();
					P->GetComponent<Transform>()->Position.x = Engine->GetGameObject("ArrowDown")->GetComponent<Transform>()->Position.x - 100.f;
					Engine->GetObjectComponent<Transform>(Engine->GetObjectByPrefix("Camera"))->SetPosition(P->GetComponent<Transform>()->Position);
				}
				else {
	...
	void End(GameObject* GO, float dt) {
		...
		if (triggered_combat) {
			...
			if (r < 30) {
				...
				MasterObjectList->ExportEnemyFilepath("./Data/Enemies/2.txt");
			}
