/******************************************************************************/
/*!
\file		Level1Controller.h
\author 	Brandon Hsu
\par    	email: b.hsu\@digipen.edu , 620007421
\date		Jan 20, 2023
\brief		Controller Script for Level 1 area 1
...
namespace Level1Controller {
	std::string SKey{ "Level1Controller" };
	...
	void Init(GameObject* GO, float dt) {
		...
		if (CT != nullptr) {
			if (MasterObjectList->StoredController != nullptr) {
				if (MasterObjectList->StoredController->LevelTag == "Level1_1") {
