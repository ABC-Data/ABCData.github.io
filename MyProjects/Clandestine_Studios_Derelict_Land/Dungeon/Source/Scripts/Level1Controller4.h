/******************************************************************************/
/*!
\file		Level1Controller4.h
\author 	Brandon Hsu
\par    	email: b.hsu\@digipen.edu , 620007421
\date		Jan 20, 2023
\brief		Controller Script for Level 1 area 4
...
namespace Level1Controller4 {
	std::string SKey{ "Level1Controller4" };
	...
	void Init(GameObject* GO, float dt) {
		...
		if (CT != nullptr) {
			if (MasterObjectList->StoredController != nullptr) {
				if (MasterObjectList->StoredController->LevelTag == "Level1_4") {
	...
	void End(GameObject* GO, float dt) {
		...
		if (triggered_combat) {
			...
			if (r) {
				...
				MasterObjectList->ExportEnemyFilepath("./Data/Enemies/3.txt");
			}
